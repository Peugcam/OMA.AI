...
class VisualAgent:
    ...
    async def _generate_scene_visual(
        ...
        # PROTEÇÃO CRÍTICA: NUNCA usar Stability AI para cenas com pessoas/rostos
        # (Stability AI gera rostos deformados, mãos com dedos extras, etc)
        people_keywords_check = ['person', 'people', 'face', 'faces', 'hand', 'hands', 'team',
                                 ...
                                 'sorrindo', 'sorriso', 'olhando', 'trabalhando', 'falando',
                                 'professor', 'estudante', 'apresentador', 'instrutor']

        desc_lower = description.lower()
        has_people_in_desc = any(keyword in desc_lower for keyword in people_keywords_check)

        if has_people_in_desc and not is_last_scene:
            self.logger.warning("🚫 BLOQUEADO: Cena com PESSOAS detectada! Stability AI gera rostos ruins.")
            self.logger.warning("   Tentando Pexels com keywords alternativas...")

            # Tentar Pexels novamente com keywords mais genéricas
            if self.pexels_api_key:
                try:
                    # Keywords super genéricas que sempre funcionam
                    generic_keywords = "business professional people working modern"
                    self.logger.info(f"🔄 Tentando Pexels com keywords genéricas: {generic_keywords}")
                    ...
                    if response.status_code == 200:
                        ...
                        if data.get("videos"):
                            ...
                            if video_files:
                                ...
                                if local_path:
                                    self.logger.info(f"✅ Pexels genérico funcionou! Evitando Stability com pessoas")
                                    return {
                                        "scene_number": scene_num,
                                        "media_path": str(local_path),
                                        "media_type": "video",
                                        "source": "pexels_generic_fallback",
                                        "duration": scene.get("duration", 5),
                                        "mood": mood,
                                        "cost": 0.0,
                    ...
                    self.logger.error(f"❌ Pexels genérico também falhou: {e}")

            # Se tudo falhou e tem pessoas, usar placeholder (NUNCA Stability para pessoas!)
            self.logger.warning("⚠️ Usando placeholder - MELHOR que Stability com pessoas!")
            placeholder_path = self._create_placeholder_image(scene_num)
            return {
                "scene_number": scene_num,
                "media_path": str(placeholder_path),
                "media_type": "image",
                "source": "placeholder_no_people_in_stability",
                "duration": scene.get("duration", 5),
                "mood": mood,
                "cost": 0.0,
                "reason": "Stability AI is BAD with human faces - using placeholder instead"
            }

        # STEP 3: Usar Stability AI (cena específica OU fallback) - APENAS SEM PESSOAS
